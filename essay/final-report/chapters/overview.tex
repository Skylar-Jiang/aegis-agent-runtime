...
\subsection{Transactional Controlled Execution}

事务式受控执行把可恢复副作用从立即可信的结果改为具有边界的生命周期。运行时在适用时建立 Checkpoint，使 Effect 先以 Pending 形态存在；只有通过必要检查和提交门后，结果才进入 Committed 状态。此机制不宣称完整 ACID 语义，也不把本地恢复等同于对不可逆外部世界副作用的撤销。其目标是在当前原型覆盖的受控资源上，明确副作用何时可见、何时可信、何时可恢复。

\subsection{Approval-aware TaskGraph Scheduling}

审批感知调度把任务组织为带依赖关系的图，并结合审批状态和 Effect target 冲突范围选择就绪节点。当高风险节点进入 \texttt{WAITING\_APPROVAL}，它的后继或冲突节点不跨越安全边界；无依赖且无冲突的独立节点继续推进。Grant 后图恢复推进；Deny 或 Cancel 后受影响范围收敛，Cancel 后 Resume 返回冲突状态。当前原型将同一节点的 speculative overlap 明确标为 \texttt{NOT\_SUPPORTED}。

\subsection{Dependency-aware Selective Rollback}

选择性回滚以 TaskGraph、Checkpoint 和 Effect 关联关系确定恢复范围。它回滚受影响依赖闭包内的 Effect，保留未进入受影响范围且已安全提交的独立 Effect；保留原因与回滚原因均写入脱敏投影和 Audit。该机制避免把部分失败简单等同于整任务恢复，也不把不可逆 Effect 伪装为可由本地状态恢复的对象。
