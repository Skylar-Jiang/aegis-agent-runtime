...
async def test_concurrent_services_create_one_grant_decision() -> None:
    ...
    results = await asyncio.gather(
        first.grant("approval-1", "reviewer-one", "approved"),
        second.grant("approval-1", "reviewer-two", "approved"),
...
@pytest.mark.asyncio
async def test_grant_rejects_blank_approver_identity() -> None:
    sf = await _init_db()
    svc = _make_service(sf)
    await svc.create(_make_approval())

    with pytest.raises(ValueError, match="nonblank approver identity"):
        await svc.grant("approval-1", "  ", "approved")
...
async def test_concurrent_services_consume_a_decision_once() -> None:
    ...
    second = _make_service(sf)
    await first.create(_make_approval())
    await first.grant("approval-1", "reviewer", "ok")

    results = await asyncio.gather(
        first.consume("approval-1"),
        second.consume("approval-1"),
        return_exceptions=True,
    )
...
async def test_concurrent_services_create_one_expired_decision() -> None:
    ...
    results = await asyncio.gather(
        first.expire("approval-1"),
        second.expire("approval-1"),
        return_exceptions=True,
    )
